...
default_index=0, orientation="horizontal",icons=None,
                ...
                "font-weight":"bold",
                "color":"#1d1160",
                "border-right":"1.5px solid #4B0082",
...
if selected_opt == 'Predictor App':
    ...
    st.markdown("<h1 style='text-align: center; color: #1E90FF;'>Loan Approval Prediction App</h1>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1:
        employment_length = st.selectbox('Choose Your Level of Expertise:', ['< 1 year', '1 year', '2 years', '3 years', '4 years', '5 years', '6 years', '7 years', '8 years', '9 years', '10+ years'])
        st.write("")
        loan_title = st.selectbox('What type of loan are you applying for?', ['Major purchase', 'Debt consolidation', 'Home improvement', 'Moving and relocation', 'Home buying', 'Business', 'Vacation', 'Car financing', 'Medical expenses', 'Credit card refinancing'])
        age = st.selectbox('Enter your age', ['< 18', '18-24', '25-34', '35-44', '45-54', '55-64', '65+'])
    with col2:
        # Modify the amount_requested and risk_score input fields with constraints
        amount_requested = st.number_input('How much do you want to borrow?', min_value=0,value=1000)
        st.write("")
        risk_score = st.number_input('Enter your Credit Score', min_value=0)
        tenure = st.selectbox('Tenure', ['36 Months','60 Months'])
    with col3:  
        debt_to_income_ratio = st.radio('Enter your Debt to income Ratio', ['Yes','No'],horizontal=True)
        if debt_to_income_ratio == 'Yes':
            st.write("")
            st.write("")
            debt_to_income_ratio = st.number_input('Enter Debt-to-Income Ratio', min_value=0.0)
        else:
            col4,col5 = st.columns(2)
            with col4:
                for i in range(2):
                    st.write("")
                monthly_debt_payments = st.text_input('Monthly Debt Payments:')
            with col5:
                for _ in range(2):
                    st.write("")
                gross_monthly_income = st.text_input('Gross Monthly Income:')
            try:
                monthly_debt_payments = float(monthly_debt_payments)
                gross_monthly_income = float(gross_monthly_income)
                # Calculate the DTI Ratio
                debt_to_income_ratio = (monthly_debt_payments / gross_monthly_income)
            except ZeroDivisionError:
                st.write("Entered Invalid Value(s)")
            for i in range(2):
                st.write("")
            st.write("Debt-to-Income Ratio:", debt_to_income_ratio)
    # Define a function to check if the data is valid
    def is_valid_data1(risk_score, amount_requested):
    ...
    if btn:
        if is_valid_data1(risk_score, amount_requested):
            ...
            # Add text annotations to display probability values on top of the bars
            for i in range(2):
                fig.add_annotation(
            ...
            st.error("Invalid data entered. Please check your inputs.")
if selected_opt =='Defaulter App':
    col1, col2 ,col3= st.columns(3)
    with col1:
      loan_amnt = st.number_input('How much amount you are looking to borrow?',value=10000)
      st.write("")
      home_ownership = st.selectbox('Select your current home ownership status:',
    ('OWN', 'RENT', 'MORTGAGE','ANY'))
    with col2:
      annual_income = st.number_input('What is your yearly earnings?', value=120000)
      st.write("")
      loan_type = st.selectbox('For what purpose do you need a loan?',
    ...
    with col3:
      int_rate=st.number_input('What interest rate are you targeting?',value=10)
      st.write("")
      credit_score=st.number_input('Feel free to provide your credit score.',value=0)
    col4,col5=st.columns(2)
    with col4:
        st.write("")
        term = st.radio(
    "Could you indicate the ideal repayment duration in months that aligns with your current financial circumstances?",
    ["36 months", "60 months"])
    with col5:
        st.write("")
        Employee_Expe = ["< 1 year","2 years","3 years","4 years","5 years","6 years","7 years","8 years","9 years","10+ years"]
        emp_lengt = st.select_slider("Choose Your Experience (in years):", options=Employee_Expe)
    ...
    if btn:
        if is_valid_data2(credit_score,loan_amnt,annual_income,int_rate):
            ...
            with col9:
                ...
                fig.update_traces(marker_line_color='black', marker_line_width=1,hovertemplate=None)
                fig.update_layout(title_text='Probability of Customer Defaulter',width=500,title_x=0.5)
                st.plotly_chart(fig,use_container_width=True)
                with col10:
                    for i in range(2):
                        st.write("")
                    ...
                    fig.update_traces(marker_line_color='black', marker_line_width=1,hovertemplate=None)
                    fig.update_layout(title_text='Top 5 Features Influencing Prediction',width=500,title_x=0.5)
...
if selected_opt=='Segmentation':
    ...
    # Column 1: Filters
    with col1:
        selected_term = st.selectbox('Ideal repayment duration:', distinct_term)
        selected_home_ownership = st.selectbox('Choose Current Home Ownership Status:', distinct_home_ownership)
    with col2:
        annual_income_input = st.number_input('Enter your Annual Income:', min_value=0,value=100000)
        loan_amount_input = st.number_input('How much Amount you want to Lend ?:', min_value=0,value=1000)
    with col3:
        selected_purpose = st.selectbox('For what type of loan you are applying for ?:', distinct_purpose)
    ...
    if btn1:
        ...
        with col9:
            ...
            fig.update_layout(title_text="Pie Chart of Approved Applications by Clusters",title_font=dict(size=20, family="Arial"))
            # Display the interactive pie chart
            st.plotly_chart(fig, use_container_width=True)
if selected_opt == 'Applications Data':
    res = session.call('LENDINGAI_DB.MART.SP_APPLICATIONSCORE_LR_VALIDATIONPROC')
    col1, col2, col3 = st.columns(3)
    with col1:
        emp_length = st.selectbox("Choose Your Level of Expertise:", ['< 1 year', '1 year', '2 years', '3 years', '4 years', '5 years', '6 years', '7 years', '8 years', '9 years', '10+ years'], key="emp_length")
        debt_to_income_ratio = st.number_input('Enter your Debt to income Ratio', min_value=0.0, key="debt_to_income_ratio")
    with col2:
        amount_requested = st.number_input('How much do you want to borrow?', min_value=0, key="amount_requested")
        loan_title = st.selectbox('What type of loan are you applying for?', ['Major purchase', 'Debt consolidation', 'Home improvement', 'Moving and relocation', 'Home buying', 'Business', 'Vacation', 'Car financing', 'Medical expenses', 'Credit card refinancing'], key="loan_title")
    with col3:
        risk_score = st.number_input('Enter your Credit Score', min_value=0, key="risk_score")
        application_status = st.selectbox("Select the application status", ['Approved', 'Rejected','Both'], key="application_status")
    col4,col5=st.columns(2)
    with col4:
        age = st.selectbox('Enter your age', ['0-18', '18-24', '25-34', '35-44', '45-54', '55-64', '65+'])
    with col5:
        tenure = st.selectbox('Select your tenure', ['36 Months','60 Months'])
    # Predict
...
if selected_opt =='Churn Data':
    col1, col2 ,col3= st.columns(3)
    with col1:
      loan_amnt = st.number_input('How much do you want to borrow?',value=10000)
      st.write("")
      home_ownership = st.selectbox('What is your current type of home ownership?',
    ('OWN', 'RENT', 'MORTGAGE','ANY'))
      st.write("")
      term = st.radio(
    "Can you specify the number of months you think would be best for repaying this loan based on your financial situation?",
    ["36 months", "60 months"])
    with col2:
      annual_income = st.number_input('How much do you earn annually?', value=120000)
      st.write("")
      loan_type = st.selectbox('What type of loan are you applying for?',
      ('Credit card refinancing','Debt consolidation','Home improvement','Major purchase','Business','Medical expenses','Moving and relocation','Vacation','Home buying','Green loan','Car financing','Other'))
      st.write("")
      Employee_Exp = ["< 1 year","2 years","3 years","4 years","5 years","6 years","7 years","8 years","9 years","10+ years"]
      emp_length = st.select_slider("Choose Your Level of Expertise:", options=Employee_Exp)
    with col3:
      int_rate=st.number_input('At what interest rate are you looking to secure your loan?',value=10)
      st.write("")
      credit_score=st.number_input('You may optionally input your credit score below:',value=0)
      st.write("")
...
if selected_opt=='Defaulter Data':
    col1, col2 ,col3= st.columns(3)
    with col1:
      loan_amount = st.number_input('What amount are you looking to borrow?',value=10000)
      st.write("")
      home_ownership = st.selectbox('What is your current home ownership status?',
    ('OWN', 'RENT', 'MORTGAGE','ANY'))
      st.write("")
      term = st.radio(
    "Optimal repayment duration in current situation?",
    ["36 months", "60 months"])
    with col2:
      annual_income = st.number_input('What is your annual income?', value=120000)
      st.write("")
      loan_type = st.selectbox('For which type of loan are you seeking approval?',
      ('Credit card refinancing','Debt consolidation','Home improvement','Major purchase','Business','Medical expenses','Moving and relocation','Vacation','Home buying','Green loan','Car financing','Other'))
      st.write("")
      Employee_Exp = ["< 1 year","2 years","3 years","4 years","5 years","6 years","7 years","8 years","9 years","10+ years"]
      emp_length = st.select_slider("Select your expertise level:", options=Employee_Exp)
    with col3:
      int_rate=st.number_input('At what interest rate do you seek the loan?',value=10)
      st.write("")
      credit_score=st.number_input('Feel free to enter your credit score.',value=0)
      st.write("")
